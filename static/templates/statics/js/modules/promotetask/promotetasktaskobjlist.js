...
var vm = new Vue({
    ...
    methods: {
        ...
        view: function (id) {
            $.get(baseURL + "promotetask/promotetasktaskobj/info/"+id, function(r){
                var param = {
                    data:r.promotetaskTaskobj
                };
                var index = layer.open({
                    title: "查看",
                    type: 2,
                    boxParams: param,
                    content: tabBaseURL + "modules/promotetask/promotetasktaskobjview.html",
                    end: function () {
                        layer.close(index);
                    }
                });
        ...
        add: function(){
            ...
            var index = layer.open({
                ...
                type: 2,
                boxParams: param,
                content: tabBaseURL + "modules/promotetask/promotetasktaskobjedit.html",
                end: function () {
                    layer.close(index);
                }
            });
        ...
        del: function (ids) {
            confirm('确定要删除选中的记录？', function(){
                $.ajax({
                    type: "POST",
                    url: baseURL + "promotetask/promotetasktaskobj/delete",
        ...
        exports: function () {
            window.location.href = urlParamByObj(baseURL + 'promotetask/promotetasktaskobj/export', vm.q);
        },
        importSuccess: function (map) {
            ...
            var index = layer.confirm(map.message, {
                ...
                if (isdownxls) {
                    ...
                    form[0].action = baseURL + 'promotetask/promotetasktaskobj/downxlserr';
                    form.submit();
                }
                layer.close(index);
                vm.reload();
...
function initUpload(upload) {
    upload.render({
        elem: '#inports',
        url: baseURL + 'promotetask/promotetasktaskobj/imports',
        field: 'file',
        auto: true,
        size: 50 * 1024 * 1024,
        accept: 'file', //普通文件
...
function initTable(table, soulTable) {
    table.render({
        ...
        url: baseURL + 'promotetask/promotetasktaskobj/list',
        where: JSON.parse(JSON.stringify(vm.q)),
        cols: [[
            {type:'checkbox'},
            {title: '操作', width:120, templet:'#barTpl',fixed:"left",align:"center"},
                            ...
                            {field:'objCode', title: '编号', minWidth:200, templet: function (d) {return isEmpty(d.objCode);}},
                            ...
                            {field:'objType', title: '目标类型：（数据字典）', minWidth:200, templet: function (d) {return isEmpty(d.objType);}},
                            {field:'objId', title: '目标主键', minWidth:200, templet: function (d) {return isEmpty(d.objId);}},
                            {field:'sourceSnapshot', title: '目标详细快照', minWidth:200, templet: function (d) {return isEmpty(d.sourceSnapshot);}},
