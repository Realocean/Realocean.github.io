...
var vm = new Vue({
    ...
    methods: {
        ...
        view: function (id) {
            $.get(baseURL + "promotetask/promotetaskpersonnel/info/"+id, function(r){
                var param = {
                    data:r.promotetaskPersonnel
                };
                var index = layer.open({
                    title: "查看",
                    type: 2,
                    boxParams: param,
                    content: tabBaseURL + "modules/promotetask/promotetaskpersonnelview.html",
                    end: function () {
                        layer.close(index);
                    }
                });
        ...
        add: function(){
            ...
            var index = layer.open({
                ...
                type: 2,
                boxParams: param,
                content: tabBaseURL + "modules/promotetask/promotetaskpersonneledit.html",
                end: function () {
                    layer.close(index);
                }
            });
        ...
        del: function (ids) {
            confirm('确定要删除选中的记录？', function(){
                $.ajax({
                    type: "POST",
                    url: baseURL + "promotetask/promotetaskpersonnel/delete",
        ...
        exports: function () {
            window.location.href = urlParamByObj(baseURL + 'promotetask/promotetaskpersonnel/export', vm.q);
        },
        importSuccess: function (map) {
            ...
            var index = layer.confirm(map.message, {
                ...
                if (isdownxls) {
                    ...
                    form[0].action = baseURL + 'promotetask/promotetaskpersonnel/downxlserr';
                    form.submit();
                }
                layer.close(index);
                vm.reload();
...
function initUpload(upload) {
    upload.render({
        elem: '#inports',
        url: baseURL + 'promotetask/promotetaskpersonnel/imports',
        field: 'file',
        auto: true,
        size: 50 * 1024 * 1024,
        accept: 'file', //普通文件
...
function initTable(table, soulTable) {
    table.render({
        ...
        url: baseURL + 'promotetask/promotetaskpersonnel/list',
        where: JSON.parse(JSON.stringify(vm.q)),
        cols: [[
            {type:'checkbox'},
            {title: '操作', width:120, templet:'#barTpl',fixed:"left",align:"center"},
                            ...
                            {field:'personnelType', title: '1员工2渠道', minWidth:200, templet: function (d) {return isEmpty(d.personnelType);}},
                            {field:'personnelId', title: '人员主键', minWidth:200, templet: function (d) {return isEmpty(d.personnelId);}},
