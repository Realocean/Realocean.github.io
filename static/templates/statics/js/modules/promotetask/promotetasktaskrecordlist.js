...
var vm = new Vue({
    ...
    methods: {
        ...
        view: function (id) {
            $.get(baseURL + "promotetask/promotetasktaskrecord/info/"+id, function(r){
                var param = {
                    data:r.promotetaskTaskrecord
                };
                var index = layer.open({
                    title: "查看",
                    type: 2,
                    boxParams: param,
                    content: tabBaseURL + "modules/promotetask/promotetasktaskrecordview.html",
                    end: function () {
                        layer.close(index);
                    }
                });
        ...
        add: function(){
            ...
            var index = layer.open({
                ...
                type: 2,
                boxParams: param,
                content: tabBaseURL + "modules/promotetask/promotetasktaskrecordedit.html",
                end: function () {
                    layer.close(index);
                }
            });
        ...
        del: function (ids) {
            confirm('确定要删除选中的记录？', function(){
                $.ajax({
                    type: "POST",
                    url: baseURL + "promotetask/promotetasktaskrecord/delete",
        ...
        exports: function () {
            window.location.href = urlParamByObj(baseURL + 'promotetask/promotetasktaskrecord/export', vm.q);
        },
        importSuccess: function (map) {
            ...
            var index = layer.confirm(map.message, {
                ...
                if (isdownxls) {
                    ...
                    form[0].action = baseURL + 'promotetask/promotetasktaskrecord/downxlserr';
                    form.submit();
                }
                layer.close(index);
                vm.reload();
...
function initUpload(upload) {
    upload.render({
        elem: '#inports',
        url: baseURL + 'promotetask/promotetasktaskrecord/imports',
        field: 'file',
        auto: true,
        size: 50 * 1024 * 1024,
        accept: 'file', //普通文件
...
function initTable(table, soulTable) {
    table.render({
        ...
        url: baseURL + 'promotetask/promotetasktaskrecord/list',
        where: JSON.parse(JSON.stringify(vm.q)),
        cols: [[
            {type:'checkbox'},
            {title: '操作', width:120, templet:'#barTpl',fixed:"left",align:"center"},
                            ...
                            {field:'taskobjId', title: '任务id', minWidth:200, templet: function (d) {return isEmpty(d.taskobjId);}},
                            {field:'targetsId', title: '指标id', minWidth:200, templet: function (d) {return isEmpty(d.targetsId);}},
                            {field:'taskpersonnelId', title: '任务员工id', minWidth:200, templet: function (d) {return isEmpty(d.taskpersonnelId);}},
                            {field:'customerOpenid', title: '客户openid', minWidth:200, templet: function (d) {return isEmpty(d.customerOpenid);}},
                            {field:'customerPhone', title: '客户手机', minWidth:200, templet: function (d) {return isEmpty(d.customerPhone);}},
                            {field:'customerNickName', title: '昵称', minWidth:200, templet: function (d) {return isEmpty(d.customerNickName);}},
                            {field:'customerName', title: '姓名', minWidth:200, templet: function (d) {return isEmpty(d.customerName);}},
                            {field:'customerAvatar', title: '头像', minWidth:200, templet: function (d) {return isEmpty(d.customerAvatar);}},
                            {field:'customerType', title: '客户性质：1新用户 2老用户', minWidth:200, templet: function (d) {return isEmpty(d.customerType);}},
                            {field:'customerPhoneModel', title: '手机型号', minWidth:200, templet: function (d) {return isEmpty(d.customerPhoneModel);}},
                            {field:'recordSnapshot', title: '浏览记录详细快照', minWidth:200, templet: function (d) {return isEmpty(d.recordSnapshot);}},
