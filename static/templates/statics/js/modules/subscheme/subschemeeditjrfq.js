    ...
    methods: {
        ...
        saveOrUpdate: function (event) {
            ...
            if(vm.subScheme.subSchemeId !=null && vm.subScheme.subSchemeId!='' ){
                //首付款
                vm.scheme.downPayment=vm.subScheme.downPayment;
                //月租
                ...
                if(vm.subScheme.randomData==null || vm.subScheme.randomData==''){
                    ...
                    //当前数据
                    //首付款
                    var downPayment=vm.subScheme.downPayment;
                    //月租
                    ...
                    if(parentData.length>0){
                        for (var i = parentData.length - 1; i >= 0; i--) {
                            if((parentData[i].downPayment==downPayment) && (parentData[i].monthlyRent==monthlyRent) && (parentData[i].tenancy==tenancy)){
                                alert("当前子方案已存在，请重新输入!");
                                return;
                            }
                        }
...
function initVerify(form) {
    form.verify({
        validate_downPayment: function (value, item) {
            if (vm.verify) {
                if (value == null || value == '') {
                    vm.verify = false;
                    return "首付款不能为空";
