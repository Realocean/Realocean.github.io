    ...
    methods: {
        ...
        saveOrUpdate: function (event) {
            ...
            if(vm.subScheme.subSchemeId !=null && vm.subScheme.subSchemeId!='' ){
                //整备费
                vm.scheme.servicingFee=vm.subScheme.servicingFee;
                //月租
                ...
                if(vm.subScheme.randomData==null || vm.subScheme.randomData==''){
                    ...
                    //当前数据
                    //整备费
                    var servicingFee=vm.subScheme.servicingFee;
                    //月租
                    ...
                    if(parentData.length>0){
                        for (var i = parentData.length - 1; i >= 0; i--) {
                            if((parentData[i].servicingFee==servicingFee) && (parentData[i].monthlyRent==monthlyRent) && (parentData[i].tenancy==tenancy)){
                                alert("当前子方案已存在，请重新输入!");
                                return;
                            }
                        }
...
function initVerify(form) {
    form.verify({
        validate_servicingFee: function (value, item) {
            if (vm.verify) {
                if (value == null || value == '') {
                    vm.verify = false;
                    return "整备费不能为空";
