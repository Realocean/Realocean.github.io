...
var vm = new Vue({
    ...
    methods: {
        ...
        view: function (id) {
            $.get(baseURL + "qmyx/scplanconfigdetail/info/"+id, function(r){
                var param = {
                    data:r.scPlanConfigDetail
                };
                var index = layer.open({
                    title: "查看",
                    type: 2,
                    boxParams: param,
                    content: tabBaseURL + "modules/qmyx/scplanconfigdetailview.html",
                    end: function () {
                        layer.close(index);
                    }
                });
        ...
        add: function(){
            ...
            var index = layer.open({
                ...
                type: 2,
                boxParams: param,
                content: tabBaseURL + "modules/qmyx/scplanconfigdetailedit.html",
                end: function () {
                    layer.close(index);
                }
            });
        ...
        del: function (ids) {
            confirm('确定要删除选中的记录？', function(){
                $.ajax({
                    type: "POST",
                    url: baseURL + "qmyx/scplanconfigdetail/delete",
        ...
        exports: function () {
            window.location.href = urlParamByObj(baseURL + 'qmyx/scplanconfigdetail/export', vm.q);
        },
        importSuccess: function (map) {
            ...
            var index = layer.confirm(map.message, {
                ...
                if (isdownxls) {
                    ...
                    form[0].action = baseURL + 'qmyx/scplanconfigdetail/downxlserr';
                    form.submit();
                }
                layer.close(index);
                vm.reload();
...
function initUpload(upload) {
    upload.render({
        elem: '#inports',
        url: baseURL + 'qmyx/scplanconfigdetail/imports',
        field: 'file',
        auto: true,
        size: 50 * 1024 * 1024,
        accept: 'file', //普通文件
...
function initTable(table, soulTable) {
    table.render({
        ...
        url: baseURL + 'qmyx/scplanconfigdetail/list',
        where: JSON.parse(JSON.stringify(vm.q)),
        cols: [[
            {type:'checkbox'},
            {title: '操作', width:120, templet:'#barTpl',fixed:"left",align:"center"},
                            ...
                            {field:'planConfigId', title: '主表id', minWidth:200, templet: function (d) {return isEmpty(d.planConfigId);}},
                            {field:'associationModeId', title: '关联方案/车型/品类/活动id', minWidth:200, templet: function (d) {return isEmpty(d.associationModeId);}},
                            {field:'associationModeName', title: '关联方案/车型/品类/活动名称', minWidth:200, templet: function (d) {return isEmpty(d.associationModeName);}},
                            {field:'delFlag', title: '删除状态(1-未删除，2-已删除)', minWidth:200, templet: function (d) {return isEmpty(d.delFlag);}},
                            ...
                            {field:'updateBy', title: '更新者', minWidth:200, templet: function (d) {return isEmpty(d.updateBy);}},
