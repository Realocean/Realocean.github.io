...
var vm = new Vue({
    ...
    methods: {
        saveOrUpdate: function (event) {
            ...
            if(vm.activity.deptName==null || vm.activity.deptName==''){
                vm.activity.deptId=null;
            }
        ...
        deptTree: function(){
            layer.open({
                ...
                btn1: function (index) {
                    ...
                    //选择上级部门
                    vm.activity.deptId = node[0].deptId;
                    vm.activity.deptName = node[0].name;
                    vm.getUserList(vm.activity.deptId);
                    layer.close(index);
                },
                end: function(){
                    $("#deptLayer").hide();
        ...
        getDept: function(){
            ...
            $.get(baseURL + "sys/dept/list", function(r){
                ...
                if(vm.activity.deptId != null && vm.activity.deptId != undefined){
                    var node = ztree.getNodeByParam("deptId", vm.activity.deptId);
                    if(node != null){
                        ztree.selectNode(node);
                        vm.activity.deptName = node.name;
                    }
                }
            })
        },
...
function initChecked(form) {
    ...
    form.on('select(handoverUserId)', function(data){
        vm.activity.sysUserId=data.value;
    });
}
