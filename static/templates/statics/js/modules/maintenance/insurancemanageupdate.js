$(function () {
    vm.detail(window.localStorage.getItem("insuranceManageId"));
    ...
    /*layui.table.render({
        ...
        cols: [[
            ...
            {field:'commercialInsuranceName',width:430, title: '商业险种'},
            {field:'amountInsured',width:430,title: '保额/万元'},
            {field:'insuranceExpenses',width:430, title: '保险费/元'}
        ]],
        page: false,
        loading: false,
...
var vm = new Vue({
    ...
    methods: {
                ...
                success: function(r){
                    if(r.code === 0){
                        alert('操作成功', function(index){
                            ...
                            parent.vm.reloadJQXSourceTable(vm.insuranceManage.carId);
                            parent.vm.reloadSYXSourceTable(vm.insuranceManage.carId);
                        });
                    }else{
                        alert(r.msg);
                    }
        ...
        reloadSourceTable: function(policyApplyNo){
            $.ajax({
                ...
                success: function(r){
                    ...
                    console.log(vm.commercialInsuranceTableList)
                    layui.table.reload('insuranceTypeGrid', {
                        data: vm.commercialInsuranceTableList
                    });
                }
        ...
        detail: function (id) {
            var type = "insuranceBj";// 保险编辑
            $.ajax({
                type: "POST",
                url: baseURL + "maintenance/insurancemanage/info/"+id+"/"+type,
                contentType: "application/json",
                data: null,
                success: function(r){
                    if(r.code === 0){
