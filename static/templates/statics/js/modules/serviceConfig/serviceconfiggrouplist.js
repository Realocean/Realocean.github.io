...
var vm = new Vue({
    ...
    methods: {
        ...
        view: function (id) {
            $.get(baseURL + "serviceconfig/serviceconfiggroup/info/"+id, function(r){
                var param = {
                ...
                var index = layer.open({
                    ...
                    type: 2,
                    boxParams: param,
                    content: baseURL + "modules/serviceconfig/serviceconfiggroupview.html",
                    end: function () {
                        layer.close(index);
                    }
                });
        ...
        add: function(){
            ...
            var index = layer.open({
                ...
                type: 2,
                boxParams: param,
                content: baseURL + "modules/serviceconfig/serviceconfiggroupedit.html",
                end: function () {
                    layer.close(index);
                }
            });
        ...
        del: function (ids) {
            confirm('确定要删除选中的记录？', function(){
                $.ajax({
                    type: "POST",
                    url: baseURL + "serviceconfig/serviceconfiggroup/delete",
        ...
        exports: function () {
            window.location.href = urlParamByObj(baseURL + 'serviceconfig/serviceconfiggroup/export', vm.q);
        },
        importSuccess: function (map) {
            ...
            var index = layer.confirm(map.message, {
                ...
                if (isdownxls) {
                    ...
                    form[0].action = baseURL + 'serviceconfig/serviceconfiggroup/downxlserr';
                    form.submit();
                }
                layer.close(index);
                vm.reload();
...
function initUpload(upload) {
    upload.render({
        elem: '#inports',
        url: baseURL + 'serviceconfig/serviceconfiggroup/imports',
        field: 'file',
        auto: true,
        size: 50 * 1024 * 1024,
        accept: 'file', //普通文件
...
function initTable(table, soulTable) {
    table.render({
        ...
        url: baseURL + 'serviceconfig/serviceconfiggroup/list',
        where: JSON.parse(JSON.stringify(vm.q)),
        cols: [[
            {type:'checkbox'},
            {title: '操作', width:120, templet:'#barTpl',fixed:"left",align:"center"},
                            ...
                            {field:'groupKey', title: '费用类型：1. 可退金额  2.违章费用 3.保险费用 4.扣款费用 5.银行信息 6.退换车说明', minWidth:200, templet: function (d) {return isEmpty(d.groupKey);}},
                            {field:'groupName', title: '费用名称', minWidth:200, templet: function (d) {return isEmpty(d.groupName);}},
