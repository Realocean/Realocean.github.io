$(function () {
    ...
    layui.table.render({
        ...
        cols: [[
            {
                field: 'receivableNo', align: "center", title: '应收单号', templet: function (d) {
                    return isEmpty(d.receivableNo);
                }
            },
            {
                field: 'operationName', align: "center", title: '操作人', templet: function (d) {
                    return isEmpty(d.operationName);
                }
            },
            {
                field: 'receivableBeforeAmount', align: "center", title: '操作前应收金额', templet: function (d) {
                    return isEmpty(d.receivableBeforeAmount);
                }
            },
            {
                field: 'uncollectBeforeAmount', align: "center", title: '操作前待收金额', templet: function (d) {
                    return isEmpty(d.uncollectBeforeAmount);
                }
            },
            {
                field: 'receivableAfterAmount', align: "center", title: '操作后应收金额', templet: function (d) {
            ...
            },
            {
                ...
                field: '', align: 'center', align: "center", title: '附件', templet: function (d) {
                    return "<span style='color:#419BEA;cursor:pointer;' onclick=fileView(\'"+d.id+"\',\'"+50+"\')>查看</span>";
                }
            },
            {
                field: 'imageUrl', align: "center", title: '附件', templet: function (d) {
                    var url="";
                    if(d.imageUrl!=null){
                        url=imageURL+d.imageUrl;
                    }
...
var vm = new Vue({
    ...
    computed: {
        ...
        brandNameAndseriesNameStr: {
            get: function () {
                return isEmpty(this.receivables.brandName)  + "/" + isEmpty(this.receivables.seriesName);
            }
        },
        paymentMethodStr: {
            get: function () {
        ...
        collectionTypeStr: {
            get: function () {
                let collectionTypeStr = getCollectionTypeStr(this.receivables.collectionType);
                if(15 == this.receivables.collectionType){
                    collectionTypeStr = this.receivables.collectionTypeName;
                } else if (0 == this.receivables.collectionType){
                    collectionTypeStr = this.receivables.collectionTypeName;
                }
        ...
        detail: function (receivablesId) {
            $.get(baseURL + "financial/receivables/skReceivablesInfo/" + receivablesId, function (r) {
                ...
                if(r.receivables.status == 3){
                    vm.detailsSupTabContentList.push('应收详情','实收详情','追帐详情', '操作记录');
                } else if(r.receivables.status == 4){
                    vm.detailsSupTabContentList.push('应收详情','实收详情','坏账详情', '操作记录');
                } else if(r.receivables.status == 5){
                    vm.detailsSupTabContentList.push('应收详情','实收详情','作废详情', '操作记录');
                } else if(r.receivables.status == 1 || r.receivables.status == 2){
                    vm.detailsSupTabContentList.push('应收详情','实收详情', '操作记录');
                }
            });
