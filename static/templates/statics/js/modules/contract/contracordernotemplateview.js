...
var vm = new Vue({
    ...
    data: {
        ...
        // 合同内容html
        contractContentUrl : null,
        fileLst: [],
        docUrl: '',
        flagSequence: sessionStorage.getItem('flagSequence')&4,
        statusStr: null
    },
    created: function () {
        var _this = this;
        ...
        if(param.contracorderNotemplate!=null){
    ...
    methods: {
        ...
        printDoc1: function () {
            ...
            // } else {
            //
            //     // var htmlUrl = location.origin+'/image-server/' + vm.contracorderNotemplate.urlHtm;
            //     // console.log('vm.contracorderNotemplate.urlHtm', vm.contracorderNotemplate.urlHtm)
            //     // htmlUrl = htmlUrl.replace(/\\/g, '/');
            //     // // 查找最后一个“/”，并截取之前的内容
            //     // var htmlUrlPrefix = htmlUrl.substring(0,htmlUrl.lastIndexOf("/"));
            //     // var htmlContent = $("#docIframe1")[0].contentWindow.document.body.outerHTML;
        ...
        replenish: function () {
            confirm('请在确保在具备签章功能且已经走完签章流程，并在合同预览中未盖章的情况下使用该功能？', function () {
                $.ajax({
                    type: "POST",
                    url: baseURL + "contract/contraccontent/sginImg/"+vm.contracorderNotemplate.id,
                    success: function (r) {
                        if (r.code == 0) {
...
function initTable(table, soulTable) {
    ...
    table.render({
        ...
        cols: [[
            {title: '操作', align:'center', width:150, templet:'#barSupplementLstTpl',fixed:"left",align:"center"},
