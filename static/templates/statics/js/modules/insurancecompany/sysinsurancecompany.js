    ...
    layui.use(['form', 'element', 'table', 'soulTable'], function () {
        ...
        //  toolbar: true,
         // defaultToolbar: ['filter'],
            url: baseURL + 'system/tsysinsurancecompany/list',
            cols: [[
                ...
                {field:'branchName', minWidth:100, title:'分公司名',align:"center",templet:function(row) {
                        if(row.branchName != null && row.branchName != '') {
...
var vm = new Vue({
    ...
    computed: {
        ...
        insuranceTypeStr:{
            get:function () {
                var str = '';
                try {
                    this.tSysInsuranceCompany.type.forEach(s => {str+=(s=='4'?' 商业保险':s=='5'?' 强险保险':'');});
                }catch (e) {
    ...
    methods: {
        ...
        add: function(){
            vm.tSysInsuranceCompany = {};
            vm.tSysInsuranceCompany.type = [];
            vm.provinceId=[];
            vm.cityId=[];
            vm.areaId=[];
            var index = layer.open({
                title: "车辆管理 > 保险公司 > 新增保险公司",
                type: 2,
                content: tabBaseURL + "modules/insurancecompany/insurancecompanyadd.html",
                end: function () {
                    layer.closeAll();
                    vm.reload();
                }
        ...
        update: function (id) {
            ...
            var index = layer.open({
                title: "车辆管理 > 保险公司 > 编辑保险公司",
                type: 2,
                content: tabBaseURL+'modules/insurancecompany/insurancecompanyedit.html',
                end: function(){
                    layer.close(index);
        ...
        del: function (ids) {
            confirm('确定要删除选中的记录？', function(){
                $.ajax({
                    type: "POST",
                    url: baseURL + "system/tsysinsurancecompany/delete",
        ...
        details: function (id,companyNo) {
            ...
            var index = layer.open({
                title: "车辆管理 > 保险公司 > 查看保险公司",
                type: 2,
                content: tabBaseURL+'modules/insurancecompany/insurancecompanydetail.html',
                end: function(){
                    layer.close(index);
                    window.localStorage.setItem("id",null);
        ...
        exports: function () {
            var url = baseURL + 'system/tsysinsurancecompany/export';
