...
var vm = new Vue({
    ...
    methods: {
        ...
        view: function (id) {
            $.get(baseURL + "order/ordercarbillinglist/info/"+id, function(r){
                var param = {
                    data:r.orderCarBillinglist
                };
                var index = layer.open({
                    title: "查看",
                    type: 2,
                    boxParams: param,
                    content: tabBaseURL + "modules/order/ordercarbillinglistview.html",
                    end: function () {
                        layer.close(index);
                    }
                });
        ...
        add: function(){
            ...
            var index = layer.open({
                ...
                type: 2,
                boxParams: param,
                content: tabBaseURL + "modules/order/ordercarbillinglistedit.html",
                end: function () {
                    layer.close(index);
                }
            });
        ...
        del: function (ids) {
            confirm('确定要删除选中的记录？', function(){
                $.ajax({
                    type: "POST",
                    url: baseURL + "order/ordercarbillinglist/delete",
        ...
        exports: function () {
            var url = baseURL + 'order/ordercarbillinglist/export?a=a';
...
function initTable(table, soulTable) {
    ...
    //  defaultToolbar: ['filter'],
        url: baseURL + 'order/ordercarbillinglist/list',
        cols: [[
            {type:'checkbox'},
            {title: '操作', width:120, templet:'#barTpl',fixed:"left",align:"center"},
                            ...
                            {field:'issue', title: '账单期号', minWidth:200, templet: function (d) {return isEmpty(d.issue);}},
                            {field:'paymentType', title: '还款类型（1、首付款 2、尾款 3、保证金 4、月租', minWidth:200, templet: function (d) {return isEmpty(d.paymentType);}},
                            {field:'repaymentDate', title: '还款日期', minWidth:200, templet: function (d) {return isEmpty(d.repaymentDate);}},
                            {field:'billPeriods', title: '账单期数', minWidth:200, templet: function (d) {return isEmpty(d.billPeriods);}},
                            {field:'repayAmount', title: '应还总金额', minWidth:200, templet: function (d) {return isEmpty(d.repayAmount);}},
