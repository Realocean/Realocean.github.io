...
var vm = new Vue({
    ...
    created: function(){
            ...
            $.get(baseURL + "order/ordercaralteration/info/" + param.id, function (r) {
                if (r.code == 0){
                    if(_this.viewTag == 'approve'|| _this.viewTag == 'reedit'||_this.viewTag == 'edit'||_this.viewTag == 'approveDetail'||_this.viewTag == 'applyDetail'){
                        ...
                        _this.initData.hasPaid = r.applyForDTO.hasPaid;
        ...
        if (_this.applyForDTO.feeItemSnapshoot && _this.applyForDTO.feeItemSnapshoot.length > 50) {
            ...
            if (_this.settlementFeeItem.hasPaid) {
                _this.settlementFeeItem.hasPaid.value = _this.initData.hasPaid;
            }
        }
    ...
    methods: {
        ...
        requestAction: function (action,type) {
            ...
            vm.applyForDTO.hasPaid = vm.initData.hasPaid;
        ...
        reeditAgain: function(){
            ...
            var index = layer.open({
                ...
                type: 2,
                boxParams: param,
                content: tabBaseURL + "modules/order/applypurchaseback.html",
                end: function () {
                    layer.close(index);
                }
            });
...
function initUpload(upload){
    upload.render({
        ...
        size: 50*1024*1024,
        accept: 'file', //普通文件
        acceptMime: '.pdf,.doc,.docx,.exl,.xlsx,.jpg,.png,.jpeg,.mp4,.zip,.rar,.mp4',
        exts: 'pdf|doc|docx|exl|xlsx|jpg|png|jpeg|mp4|zip|rar|mp4', //
        multiple: true,
        number:20,
        choose: function(obj){
            PageLoading();
