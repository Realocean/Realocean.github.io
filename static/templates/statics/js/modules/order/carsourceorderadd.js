$(function () {
    ...
    layui.use(['form', 'layedit', 'laydate', 'element', 'table', 'soulTable'], function(){
        ...
        layui.form.on('select(sourceTypeSelect)', function (data) {
            vm.carSourceOrder.carSourceType = data.value;
            vm.carSourceType = data.value;
            var type;
            if(vm.carSourceType == '1'){
                type = "7";
            } else if(vm.carSourceType == '2'){
                type = "6";
            } else {
                type = "8";
            }
            ...
            if(vm.carSourceType == '1' || vm.carSourceType == '2'){
                $.get(baseURL + "order/orderfeeitemdict/selectorByRentType/" + type, function (r) {
                    vm.feeItemLst = r.datas;
                });
            } else {
            ...
            if(data.value == '1'){//经租
                ...
                $("#ghmodel").hide();
                $("#totalPrice").hide();
                vm.carSourceOrder.purchaseDate = "";
                vm.carSourceOrder.rentStartDate = "";
                vm.carSourceOrder.rentEndDate = "";
                vm.carSourceOrder.rentMonthPrice = "";
                vm.carSourceOrder.refundType = "";
                vm.carSourceOrder.payDate = "";
                vm.carSourceOrder.totalPrice = null;
                layui.form.render();
            } else if(data.value == '2'){//直购
                $("#jzmodel").hide();
                $("#ghmodel").hide();
                $("#totalPrice").show();
                vm.carSourceOrder.purchaseDate = "";
...
var vm = new Vue({
    ...
    created: function(){
        ...
        var _this = this;
        console.log(_this.carSourceOrder.carSourceType);
    ...
    methods: {
            ...
            var index = layer.open({
                ...
                content: tabBaseURL + "modules/common/selectcar.html",
                    ...
                    if(carId!=null && carId!='' && carId!=undefined){
                        ...
                        // 获取车辆数据之前校验该车是否已有用车中的车辆来源订单，有的话给用户提示，否则不提示
                        vm.carSourceOrder.carId = carId;
                        $.ajax({
                            type: "POST",
                            url: baseURL + "pay/carsourceorder/checkCarIsOrder",
                            contentType: "application/json",
                            data: JSON.stringify(vm.carSourceOrder),
                            success: function(r){
                                if(r.code == 0){
                                    alert("该车辆已有用车中的订单来源数据！");
                                } else {
                                    // 根据车辆ID获取车辆来源订单采购方案
                                    vm.getCarBasicSourcePlan(carId);
                                }
                            }
                        });
                    }
        ...
        //根据车辆ID查询采购方案
        getCarBasicSourcePlan:function (carId) {
            $.ajax({
                type: "POST",
                url: baseURL + "pay/carsourceorder/getCarBasicSourcePlan/"+carId,
                contentType: "application/json",
                data: {},
                success: function(r){
                    if(r.data != null){
                        vm.carSourceOrder.contractId =r.data.contractId;
                        vm.carSourceOrder.contractNo =r.data.contractNo;
                        vm.carSourceOrder.supplierId =r.data.supplierId;
                        vm.carSourceOrder.supplierName =r.data.supplierName;
                        vm.carSourceOrder.purchaseDate =r.data.handleDate;
                        vm.carSourceOrder.refundType = r.data.repayment;
                        vm.carSourceOrder.rentStartDate = r.data.rentStartDate;
                        vm.carSourceOrder.rentEndDate = r.data.rentEndDate;
                        vm.carSourceOrder.rentMonthPrice = r.data.rentMonthPrice;
                        vm.carSourceOrder.payDate = r.data.dueDate;
                        vm.carSourceOrder.payType = r.data.payType;
                        vm.carSourceOrder.amountPaid = r.data.amountPaid;
                        vm.carSourceOrder.transferDate = r.data.transferTime;
                        vm.carSourceOrder.orderRemark = r.data.remark;
                        // vm.carSourceOrder.carSourceType = r.data.carSourceType;
                        if(r.data.feeItemLst != null){
                            vm.orderRelerecord.feeLst = r.data.feeItemLst;
                            vm.reloadFeeItem();
                        }
                        if(r.data.basicSourceFileLst != null){
                            vm.fileList = r.data.basicSourceFileLst;

                        }

                        if(r.data.carSourceType == 1){// 直购
                            vm.carSourceOrder.carSourceType = 2;
                        } else if(r.data.carSourceType == 2){//经租
                            vm.carSourceOrder.carSourceType = 1;
                        } else {
                            vm.carSourceOrder.carSourceType = 3;//以租代购
                        }
                        if(vm.carSourceOrder.carSourceType == '1'){//经租
                            $("#jzmodel").show();
                            $("#ghmodel").hide();
                            $("#totalPrice").hide();
                        } else if(vm.carSourceOrder.carSourceType == '2'){//直购
                            $("#jzmodel").hide();
                            $("#ghmodel").hide();
                            $("#totalPrice").show();
                        } else if(vm.carSourceOrder.carSourceType == '3'){//以租代购
                            $("#jzmodel").show();
                            $("#ghmodel").show();
                            $("#totalPrice").hide();
                        }
                        ...
                        vm.carSourceOrder = Object.assign({}, vm.carSourceOrder,{
                            ...
                            handleDate:r.data.handleDate,
                            supplierId:r.data.supplierId,
                            supplierName:r.data.supplierName,
                            refundType : r.data.repayment,
                            rentStartDate : r.data.rentStartDate,
                            rentEndDate : r.data.rentEndDate,
                            rentMonthPrice : r.data.rentMonthPrice,
                            payDate : r.data.dueDate,
                            payType : r.data.payType,
                            amountPaid : r.data.amountPaid,
                            transferDate : r.data.transferTime,
                            carSourceType : vm.carSourceOrder.carSourceType,
                            orderRemark : r.data.remark,
                        });

                        vm.carSourceType = vm.carSourceOrder.carSourceType;

                        // 获取费用项
                        var type;
                        if(vm.carSourceOrder.carSourceType == '1'){
                            type = "7";
                        } else if(vm.carSourceOrder.carSourceType == '2'){
                            type = "6";
                        } else {
                            type = "8";
                        }
                        ...
                        vm.carSourceOrder.refundType = "";
                        vm.carSourceOrder.payDate = "";
                        vm.carSourceOrder.contractId ="";
                        vm.carSourceOrder.contractNo ="";
                        vm.carSourceOrder.supplierId ="";
                        vm.carSourceOrder.supplierName ="";
                        vm.carSourceOrder.payDate ="";
                        vm.carSourceOrder.payType ="";
                        vm.carSourceOrder.amountPaid ="";
                        vm.carSourceOrder.transferDate ="";
                        vm.carSourceOrder.orderRemark = "";
                        vm.orderRelerecord.feeLst = [];
                        vm.fileList = [],
        ...
        saveOrUpdate: function (event) {
            ...
            vm.carSourceOrder.carSourceType = value;
            ...
            if(vm.carSourceOrder.carSourceType == 1 || vm.carSourceOrder.carSourceType == 3){
