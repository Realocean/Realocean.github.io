...
var vm = new Vue({
    ...
    methods: {
        ...
        view: function (id) {
            $.get(baseURL + "order/orderpaymenfeeitem/info/"+id, function(r){
                var param = {
                    data:r.orderPaymenfeeitem
                };
                var index = layer.open({
                    title: "查看",
                    type: 2,
                    boxParams: param,
                    content: tabBaseURL + "modules/order/orderpaymenfeeitemview.html",
                    end: function () {
                        layer.close(index);
                    }
                });
        ...
        add: function(){
            ...
            var index = layer.open({
                ...
                type: 2,
                boxParams: param,
                content: tabBaseURL + "modules/order/orderpaymenfeeitemedit.html",
                end: function () {
                    layer.close(index);
                }
            });
        ...
        del: function (ids) {
            confirm('确定要删除选中的记录？', function(){
                $.ajax({
                    type: "POST",
                    url: baseURL + "order/orderpaymenfeeitem/delete",
        ...
        exports: function () {
            window.location.href = urlParamByObj(baseURL + 'order/orderpaymenfeeitem/export', vm.q);
        },
        importSuccess: function (map) {
            ...
            var index = layer.confirm(map.message, {
                ...
                if (isdownxls) {
                    ...
                    form[0].action = baseURL + 'order/orderpaymenfeeitem/downxlserr';
                    form.submit();
                }
                layer.close(index);
                vm.reload();
...
function initUpload(upload) {
    upload.render({
        elem: '#inports',
        url: baseURL + 'order/orderpaymenfeeitem/imports',
        field: 'file',
        auto: true,
        size: 50 * 1024 * 1024,
        accept: 'file', //普通文件
...
function initTable(table, soulTable) {
    table.render({
        ...
        url: baseURL + 'order/orderpaymenfeeitem/list',
        where: JSON.parse(JSON.stringify(vm.q)),
        cols: [[
            {type:'checkbox'},
            {title: '操作', width:120, templet:'#barTpl',fixed:"left",align:"center"},
                            ...
                            {field:'feeitemCode', title: '', minWidth:200, templet: function (d) {return isEmpty(d.feeitemCode);}},
                            {field:'orderPaymentId', title: '订单支付信息id', minWidth:200, templet: function (d) {return isEmpty(d.orderPaymentId);}},
                            ...
                            {field:'couponNo', title: '优惠券编号', minWidth:200, templet: function (d) {return isEmpty(d.couponNo);}},
                            {field:'couponFaceValue', title: '优惠券面额', minWidth:200, templet: function (d) {return isEmpty(d.couponFaceValue);}},
                            {field:'couponUseValue', title: '优惠券使用金额', minWidth:200, templet: function (d) {return isEmpty(d.couponUseValue);}},
                            {field:'couponType', title: '优惠券类型', minWidth:200, templet: function (d) {return isEmpty(d.couponType);}},
                            {field:'couponData', title: '优惠券信息', minWidth:200, templet: function (d) {return isEmpty(d.couponData);}},
                            {field:'couponStatus', title: '优惠券状态：1.已使用,2、已退回，3、已作废', minWidth:200, templet: function (d) {return isEmpty(d.couponStatus);}},
                            {field:'deductionAmountSurplus', title: '剩余可抵扣金额', minWidth:200, templet: function (d) {return isEmpty(d.deductionAmountSurplus);}},
                            {field:'deductedAmount', title: '已抵扣金额', minWidth:200, templet: function (d) {return isEmpty(d.deductedAmount);}},
