$(function () {
    ...
    if (vm.settleType == 1) {
        ...
        layui.laydate.render({
            ...
            type: 'date',
            trigger: 'click',
            min:vm.applyForDTO.currTimeStartRent,
        ...
        layui.laydate.render({
            ...
            // min:minTime,
            trigger:'click',
            min:vm.applyForDTO.timeStartRent,
            max:vm.applyForDTO.timeFinishRent,
            done: function (value) {
                vm.applyForDTO.timeApply = value;
            }
    ...
    layui.upload.render({
        ...
        choose: function (obj) {
            ...
            console.log(vm.deliveryFileLst);
        },
        error: function () {
            RemoveLoading();
            layer.msg('上传失败', {icon: 5});
...
var vm = new Vue({
    ...
    created: function(){
        ...
        if(param.viewTag != 'apply'){
            ...
            $.get(baseURL + "order/ordercaralteration/info/" + param.id, function (r) {
                if (r.code == 0){
                    if(_this.viewTag == 'approve'|| _this.viewTag == 'reedit' ||_this.viewTag == 'edit'||_this.viewTag == 'approveDetail'||_this.viewTag == 'applyDetail'){
                        ...
                        if(_this.applyForDTO.currTimeStartRent == null || _this.applyForDTO.currTimeStartRent == undefined){
                            _this.applyForDTO.currTimeStartRent = _this.applyForDTO.timeStartRent;
                        }
            ...
            if(window.localStorage.getItem("code")!='' && window.localStorage.getItem("code")!=null){
                $.get(baseURL + "order/ordercaralteration/detailData/" + window.localStorage.getItem("code"), function (r) {
                    ...
                    if (_this.order.orderCar.timeStartRent) {
                        _this.applyForDTO.timeStartRent = dateFormatYMD(_this.order.orderCar.timeStartRent);
                    }
                    if (_this.order.orderCar.timeFinishRent) {
                        _this.applyForDTO.timeFinishRent = dateFormatYMD(_this.order.orderCar.timeFinishRent);
                    }
                    _this.applyForDTO.currTimeStartRent = _this.order.orderCar.currTimeStartRent == null? _this.applyForDTO.timeStartRent:_this.order.orderCar.currTimeStartRent;
        ...
        if (_this.applyForDTO.feeItemSnapshoot && _this.applyForDTO.feeItemSnapshoot.length > 50) {
            ...
            _this.feeItemSnapshoot.view = initSettlementFeeItemView(_this.nodeFields, _this.settlementFeeItem, 'transferCar_carChange_', {
                suitType: 2,
                suitRentType: 1
            });
            ...
            if (_this.settlementFeeItem.rentAmount) {
        ...
        visibleStateModification(_this.nodeFields, 'transferCar_carChange_serviceConfig');
    ...
    methods: {
        ...
        requestAction: function (action,type) {
            PageLoading();
            var closeBox = false;
            if(vm.nodeFields.transferCar_carChange_shipType.hide){
        ...
        reeditAgain: function(){
            ...
            var index = layer.open({
                ...
                type: 2,
                boxParams: param,
                content: tabBaseURL + "modules/order/applytransfercar.html",
                end: function () {
                    layer.close(index);
                }
            });
        ...
        saveOrUpdate: function () {
            ...
            if(vm.nodeFields.transferCar_carChange_uploadAttach.required){
                if(vm.deliveryFileLst.length==0){
                    alert('上传附件为空');
                    return false;
                }
            }
            if(vm.nodeFields.transferCar_carChange_shipType.required){
            ...
            vm.applyForDTO.alterationType = 1;
            vm.applyForDTO.processApproveForm = {
                processKey : vm.processKey,
                instanceId : vm.instanceId,
                reedit : vm.reedit
...
function monthlyRentDebt(value) {
    vm.applyForDTO.timeApply = value;
    var urlPath = baseURL + "order/ordercaralteration/monthlyRentDebt/"+vm.applyForDTO.orderCarCode+"/"+new Date(value).getTime();
    // if(vm.settleType == 1){
    //     urlPath = baseURL + "order/ordercaralteration/transferMonthlyRentDebt/"+vm.applyForDTO.orderCarCode+"/"+new Date(value).getTime();
    // }
    if (returncar_rent_settlement && returncar_rent_settlement_model == 1 && vm.applyForDTO.rentType != 8) {
