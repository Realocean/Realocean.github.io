...
var vm = new Vue({
    ...
    methods: {
        saveOrUpdate: function (event) {
            var url = vm.scWriteOffGoods.id == null ? "writeoff/scwriteoffgoods/save" : "writeoff/scwriteoffgoods/update";
            PageLoading();
            $.ajax({
                type: "POST",
                url: baseURL + url,
                contentType: "application/json",
                data: JSON.stringify(vm.scWriteOffGoods),
                success: function(r){
                    RemoveLoading();
                    if(r.code === 0){
...
function initVerify(form) {
    form.verify({
                ...
                if (vm.verify) {
                    ...
                    validate_writeOffLogId: function (value, item) { //value：表单的值、item：表单的DOM对象
                if (vm.verify) {
                    if (value == null || value == '') {
                        vm.verify = false;
                        return "核销表id不能为空";
