...
var vm = new Vue({
    ...
    methods: {
        ...
        view: function (id) {
            $.get(baseURL + "writeoff/scwriteoffgoods/info/"+id, function(r){
                var param = {
                    data:r.scWriteOffGoods
                };
                var index = layer.open({
                    title: "查看",
                    type: 2,
                    boxParams: param,
                    content: tabBaseURL + "modules/writeoff/scwriteoffgoodsview.html",
                    end: function () {
                        layer.close(index);
                    }
                });
        ...
        add: function(){
            ...
            var index = layer.open({
                ...
                type: 2,
                boxParams: param,
                content: tabBaseURL + "modules/writeoff/scwriteoffgoodsedit.html",
                end: function () {
                    layer.close(index);
                }
            });
        ...
        del: function (ids) {
            confirm('确定要删除选中的记录？', function(){
                $.ajax({
                    type: "POST",
                    url: baseURL + "writeoff/scwriteoffgoods/delete",
        ...
        exports: function () {
            window.location.href = urlParamByObj(baseURL + 'writeoff/scwriteoffgoods/export', vm.q);
        },
        importSuccess: function (map) {
            ...
            var index = layer.confirm(map.message, {
                ...
                if (isdownxls) {
                    ...
                    form[0].action = baseURL + 'writeoff/scwriteoffgoods/downxlserr';
                    form.submit();
                }
                layer.close(index);
                vm.reload();
...
function initUpload(upload) {
    upload.render({
        elem: '#inports',
        url: baseURL + 'writeoff/scwriteoffgoods/imports',
        field: 'file',
        auto: true,
        size: 50 * 1024 * 1024,
        accept: 'file', //普通文件
...
function initTable(table, soulTable) {
    table.render({
        ...
        url: baseURL + 'writeoff/scwriteoffgoods/list',
        where: JSON.parse(JSON.stringify(vm.q)),
        cols: [[
            {type:'checkbox'},
            {title: '操作', width:120, templet:'#barTpl',fixed:"left",align:"center"},
                            ...
                            {field:'writeOffLogId', title: '核销表id', minWidth:200, templet: function (d) {return isEmpty(d.writeOffLogId);}},
                            {field:'shoppingServiceName', title: '商品名称', minWidth:200, templet: function (d) {return isEmpty(d.shoppingServiceName);}},
                            {field:'price', title: '单价', minWidth:200, templet: function (d) {return isEmpty(d.price);}},
                            {field:'number', title: '核销数量', minWidth:200, templet: function (d) {return isEmpty(d.number);}},
                            ...
                            {field:'updateBy', title: '修改人', minWidth:200, templet: function (d) {return isEmpty(d.updateBy);}},
