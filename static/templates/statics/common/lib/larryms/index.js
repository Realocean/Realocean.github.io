...
layui.define(['jquery','form','larryTab','laytpl','larry'],function(exports){
    var $ = layui.$,
        ...
        $win = $(window),
        $body = $('body');
     window.larryTab = larryTab;
     console.log('window.larryTab',window.larryTab)
    //系统锁屏控制
    ...
    // 用户信息
    $('#userInfo').off('click').on('click',function(e){
        var sysUserInfo = window.localStorage.getItem("sysUserInfo");
        var jsonStr = JSON.parse(sysUserInfo)||{};
        document.getElementById("username").innerText = jsonStr.username;
        document.getElementById("lastLoginTime").innerText = jsonStr.lastLoginTime;
        $('#useravatar > img').remove();
        if (sessionStorage.getItem('avatar') != null){
            var url = imageURL + sessionStorage.getItem('avatar');
            $('#useravatar').prepend('<img class="info-img" src="'+url+'" alt="">');
        }else {
            $('#useravatar').prepend('<img class="info-img" src="${request.contextPath}/statics/images/user.jpg" alt="">');
        }
        e.stopPropagation()
        $('#u-personalInformation').toggle();
    })
    // 个人信息
    $('#personal').click(function(){
    $('#u-personalInformation').hide();
        var index = layer.open({
                        title: "个人中心",
                        type: 2,
                        area:['800px','480px'],
    ...
    $(document).click(function(e){
        let _com = $('#u-personalInformation');
        if(!_com.is(e.target) && _com.has(e.target).length==0){
            $('#u-personalInformation').hide()
        }
    })
