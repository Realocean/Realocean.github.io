...
layui.define(['larryms','larryElem'],function(exports){
     ...
     LarryTab = function() {
         ...
         this.config = {
         	...
         	tabSession:false,//是否开启tab选项卡缓存
         	closed:true,//选项卡是否包含删除按钮
         	tabMax:25,//默认允许最大打开选项卡个数
         	autoRefresh:true,//选项卡重新打开时自动刷新操作
        ...
        if(index.tabIndex == -1){
           ...
           if(index.pageIndex == undefined){
            //导航菜单新增
              ...
              title += '<cite index="'+data.index+'">'+data.title+'</cite>';
              if(_config.closed){
                 title +='<i class="layui-icon layui-unselect layui-tab-close" data-id="'+ full_tabIndex +'" data-domId="'+data.domId+'">&#x1006;</i>';
              }
              // iframe方式添加
              ...
              larryElem.tabAdd(ELEM.tabFilter,{
               ...
               larryElem.boxParams=data.params;
               if (!layer.boxParams)layer.boxParams = {};
              // 新增tab后的系列动作 
    ...
    LarryTab.prototype.recoveryTab = function(tab){
        ...
        if(index.tabIndex == -1){
            ...
            title += '<cite>'+tab.title+'</cite>';
              ...
              // iframe方式添加
             var content = '<iframe src="'+tab.href+'" id="ifr'+tab.id+'" data-group="'+tab.group+'" data-domId="'+tab.domId+'" data-id="'+tab.LarryID+'" lay-id="'+tab.id+'" name="ifr_'+tab.id+'" class="larryms-iframe"></iframe>';
              // tab添加
              larryElem.tabAdd(ELEM.tabFilter,{
                  title: title,
                  content: content,
    ...
    function getHtml(data,status){
    	...
    	if(status == 'on'){
            ...
            for(var i = 0 ;i<data.length;i++){
            	...
            	navHtml.top +='<a data-group="' + i + '" data-domId="'+data[i].domId+'" data-id="larry-'+data[i].id+'">';
