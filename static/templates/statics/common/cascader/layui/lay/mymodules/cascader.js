...
layui.define(["jquery","laytpl","layer"], function (exports) {
    ...
    Cascader.prototype={
        ...
        // 初始化容器和标签
        init: function () {
            $(this.elem).after('<i class="layui-icon layui-icon-down cascader" data-cascader="icon"></i>');
            ...
            var string =  laytpl(
                '<ul class="urp-cascader-child">'+
                    '{{# for(var i=0;i<d.length;i++){ }}'+
                        '<li>{{ d[i].label }}<i class="layui-icon layui-icon-right cascader" ></i></li>'+
                    '{{# } }}'+
                '</ul>'
            ).render(this.d);
        ...
        finishInitData: function (triggerData) {
            ...
            $(this.elem).siblings("i").replaceWith('<i class="layui-icon layui-icon-down cascader" data-cascader="icon"></i>');
        ...
        callback: function () {
            ...
            $(document).on("click",self.elem+","+self.elem+" ~ i.layui-icon", function () {
                ...
                if (self.onOff) {
                    ...
                    $(self.elem).siblings("i").replaceWith('<i class="layui-icon layui-icon-up cascader" data-cascader="icon"></i>');

                    self.domContent.css("zIndex",zIndex);
                } else {
                    $(self.elem).siblings(".urp-cascader-content").find("ul").slideUp(100);
                    $(self.elem).siblings("i").replaceWith('<i class="layui-icon layui-icon-down cascader" data-cascader="icon"></i>');
                }
            })
            // 点击外层文档隐藏
